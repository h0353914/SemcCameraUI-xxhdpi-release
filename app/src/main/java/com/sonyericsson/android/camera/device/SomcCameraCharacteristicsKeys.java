...
public final class SomcCameraCharacteristicsKeys {
    // Restored to Boolean.TYPE to match Smali version which works correctly
    ...
    // Restored to Boolean.TYPE to match Smali version
    ...
    private static <T> CameraCharacteristics.Key<T> keyConstructor(String str, Class<T> cls) {
        try {
            return (CameraCharacteristics.Key<T>) Class.forName("android.hardware.camera2.CameraCharacteristics$Key").getConstructor(String.class, Class.class).newInstance(str, cls);
