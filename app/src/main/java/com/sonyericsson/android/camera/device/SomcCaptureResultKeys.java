...
public final class SomcCaptureResultKeys {
    ...
    private static <T> CaptureResult.Key<T> keyConstructor(String str, Class<T> cls) {
        try {
            return (CaptureResult.Key<T>) Class.forName("android.hardware.camera2.CaptureResult$Key").getConstructor(String.class, Class.class).newInstance(str, cls);
