...
class FilesKt__UtilsKt extends FilesKt__FileTreeWalkKt {
    ...
    */
    public static final boolean copyRecursively(@NotNull File receiver, @NotNull File target, boolean z, @NotNull Function2<? super File, ? super IOException, ? extends OnErrorAction> onError) {
        ...
        if (!receiver.exists()) {
            return onError.invoke(receiver, new NoSuchFileException(receiver, null, "The source file doesn't exist.", 2, null)) != OnErrorAction.TERMINATE;
        }
        try {
            for (File file : FilesKt.walkTopDown(receiver).onFail(new FilesKt__UtilsKt$copyRecursively$2(onError))) {
                if (!file.exists()) {
                    if (onError.invoke(file, new NoSuchFileException(file, null, "The source file doesn't exist.", 2, null)) == OnErrorAction.TERMINATE) {
                        return false;
                    }
                } else {
                    File file2 = new File(target, FilesKt.toRelativeString(file, receiver));
